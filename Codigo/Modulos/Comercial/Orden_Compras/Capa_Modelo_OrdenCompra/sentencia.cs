    ...
    public class sentencia
    {
        ...
        */
        ...
        public bool InsertOrder(string numeroOC, int proveedorId, DateTime fechaEntrega, decimal total, List<(int codigoProducto, decimal precioU, int cantidad, decimal totalDetalle)> detalles)
        {
            ...
            using (OdbcConnection conn = cn.conectar()) // Aquí asumo que `cn.conectar()` retorna una instancia de `OdbcConnection`
            {
                ...
                try
                {
                    ...
                    using (var transaction = conn.BeginTransaction())
                    {
                        try
                        {
                            Console.WriteLine("Comenzando transacción...");

                            // Query para insertar el encabezado de la orden de compra
                            string encabezadoQuery = "INSERT INTO Tbl_EncabezadoOrdenCompra (PK_encOrCom_numeroOC, Fk_prov_id, EncOrCom_fechaEntrega, total) " +
                                                     "VALUES (?, ?, ?, ?)";

                            // Ejecutamos el comando para insertar el encabezado dentro de la transacción
                            using (var cmdEncabezado = new OdbcCommand(encabezadoQuery, conn, transaction))
                            {
                                ...
                                cmdEncabezado.Parameters.AddWithValue("?", fechaEntrega);
                                cmdEncabezado.Parameters.AddWithValue("?", total);
                                cmdEncabezado.ExecuteNonQuery(); // Ejecuta el comando de inserción del encabezado
                            }

                            Console.WriteLine("Encabezado insertado, ahora insertando detalles...");

                            // Query para insertar cada detalle de la orden de compra
                            ...
                            // Recorremos cada detalle en la lista y lo insertamos uno por uno
                            foreach (var detalle in detalles)
                            {
                                using (var cmdDetalle = new OdbcCommand(detalleQuery, conn, transaction))
                                {
                                    ...
                                    cmdDetalle.ExecuteNonQuery(); // Ejecuta el comando de inserción de detalle
                                }
                            }

                            Console.WriteLine("Detalles insertados, confirmando transacción...");

                            // Confirmamos la transacción si todo es exitoso
                            transaction.Commit();
                            return true; // Retorna true si la inserción es exitosa
                        }
                        catch (OdbcException ex)
                        {
                            LastErrorMessage = ex.Message; // Captura el mensaje de error específico de ODBC
                        ...
                        {
                            LastErrorMessage = ex.Message; // Captura cualquier otro tipo de error
                ...
                catch (Exception ex)
                {
                    ...
                    return false; // Si no se puede abrir la conexión, retorna false
                }
                finally
                {
                    conn.Close(); // Asegura que la conexión se cierre en cualquier caso
